...
def test_labels(tmp_path):
    ...
    with ArgvContext('promtext', '--label', 'testlabel=testvalue', str(promfile), 'test_metric', 0):
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP test_metric metric appended by promtext-cli" "\n" 
...
def test_existing_metric_append_metric(tmp_path):
    """
    Does the command append a new metric to an existing file with metrics content and preserve existing metrics?
    """
    ...
    with ArgvContext('promtext', '--label', 'testlabel=testvalue', str(promfile), 'new_metric', 0):
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
...
def test_existing_metric_append_labelvalue(tmp_path):
    ...
    with ArgvContext('promtext', '--label', 'testlabel=new', str(promfile), 'existing_metric', 0):
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
...
def test_existing_metric_multilabel(tmp_path):
    ...
    with ArgvContext('promtext', '--label', 'first=new', '--label', 'second=new', str(promfile), 'existing_metric', "42"):
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
...
def test_existing_metric_overwrite(tmp_path):
    ...
    with ArgvContext('promtext', '--label', 'testlabel=existing', str(promfile), 'existing_metric', "42"):
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
...
def test_existing_metric_labeldrop(tmp_path, capsys):
    ...
    with ArgvContext('promtext', str(promfile), 'existing_metric', "42"), pytest.raises(SystemExit) as pytest_wrapped_e:
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
    ...
    assert "ERROR:promtext_cli.main:previously known label 'testlabel' missing, cannot update!" in captured.err
...
def test_existing_metric_labelchange(tmp_path, capsys):
    ...
    with ArgvContext('promtext','--label', 'newlabel=new', str(promfile), 'existing_metric', "42"), pytest.raises(SystemExit) as pytest_wrapped_e:
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
    ...
    assert pytest_wrapped_e.value.code == 1
    captured = capsys.readouterr()
    assert "ERROR:promtext_cli.main:previously known label 'existinglabel' missing, cannot update!" in captured.err
...
def test_existing_metric_labeladd(tmp_path, capsys):
    ...
    with ArgvContext('promtext','--label', 'existinglabel=existing', '--label', 'newlabel=new', str(promfile), 'existing_metric', "42"), pytest.raises(SystemExit) as pytest_wrapped_e:
        promtext_cli.main.promtext()
    assert promfile.exists()
    assert promfile.read_text() == (
        "# HELP existing_metric metric appended by promtext-cli" "\n" 
    ...
    assert pytest_wrapped_e.value.code == 1
    captured = capsys.readouterr()
    assert "ERROR:promtext_cli.main:labelnames for metric existing_metric not the same, cannot update" in captured.err
